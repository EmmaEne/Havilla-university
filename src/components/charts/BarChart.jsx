        ...
        <div className="custom-chart-tooltip-items">
          {payload.map((entry, index) => (
            <div key={index} className="custom-chart-tooltip-item">
              <span style={{ color: entry.color, display: 'flex', alignItems: 'center', gap: '4px' }}>
...
export default function BarChart({ data, dataKey, xKey, color = 'var(--color-primary)', height = 300, name = 'Value' }) {
      ...
      <ResponsiveContainer width="100%" height="100%">
        <RechartsBarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <YAxis
            ...
            axisLine={false}
          />
          <Tooltip content={<CustomTooltip />} cursor={{ fill: isDark ? 'rgba(37, 51, 71, 0.3)' : 'rgba(241, 245, 249, 0.5)' }} />
          <Bar
            dataKey={dataKey}
            name={name}
            fill={resolvedColor}
